...
@dataclass
class ControlNetAudioLDM2Output(BaseOutput):
    """
    The output of [`ControlNetAudioLDM2Model`].
...
class ControlNetAudioLDM2Model(ModelMixin, ConfigMixin):
    """
    A ControlNet for AudioLDM2 model.
    ...
    def __init__(
        ...
        conv_in_cond_kernel = 3
        conv_in_cond_padding = (conv_in_kernel - 1) // 2
        self.controlnet_cond_embedding = nn.Conv2d(
            in_channels, block_out_channels[0], kernel_size=conv_in_cond_kernel, padding=conv_in_cond_padding
        )

        # self.controlnet_cond_embedding = ControlNetAudioLDM2ConditioningEmbedding(
    ...
    def from_unet(
        ...
        conditioning_embedding_out_channels: Optional[Tuple[int, ...]] = (256, 512, 1024, 2048),
        load_weights_from_unet: bool = True,
        conditioning_channels: int = 124,
    ):
        r"""
    ...
    def forward(
    ...
    ) -> Union[ControlNetAudioLDM2Output, Tuple[Tuple[torch.FloatTensor, ...], torch.FloatTensor]]:
        """
        ...
        return ControlNetAudioLDM2Output(
...
if __name__ == '__main__':
    from main.data.dataset_musdb import create_musdb_dataset

    ds = create_musdb_dataset(path="../../data/musdb18hq/train.tar",
                             sample_rate=16000, chunk_dur=10.0, shardshuffle=False)
    wav, stems = next(iter(ds))
    unet = AudioLDM2UNet2DConditionModel.from_pretrained("cvssp/audioldm2-large", subfolder="unet")
    controlnet = ControlNetAudioLDM2Model.from_unet(unet)
